...
public class CopyOfMAHLIClient extends RoverClientRunnable {
	...
	public CopyOfMAHLIClient(int port, InetAddress host) throws UnknownHostException {
		super(port, host);
	}
	...
	public void run() {
		try {
			...
			/*
			 * //Send 5 messages to the Server for(int i = 0; i < 5; i++){
			 * //write to socket using ObjectOutputStream outputToAnotherObject
			 * = new ObjectOutputStream(getRoverSocket().getNewSocket().
			 * getOutputStream());
			 * 
			 * System.out.println(
			 * "=================================================");
			 * System.out.println(
			 * "Module MAHLI Client: Sending request to MAHLI SERVER");
			 * System.out.println(
			 * "=================================================");
			 * 
			 * if(i == 4){ outputToAnotherObject.writeObject("exit"); } else {
			 * outputToAnotherObject.writeObject("Message #" + i +
			 * " from module MAHLI."); }
			 * 
			 * //read the server response message inputFromAnotherObject = new
			 * ObjectInputStream(getRoverSocket().getSocket().getInputStream());
			 * String message = (String) inputFromAnotherObject.readObject();
			 * System.out.println("Module 1 Client: Message from Server - " +
			 * message.toUpperCase());
			 * 
			 * //close resources inputFromAnotherObject.close();
			 * outputToAnotherObject.close(); Thread.sleep(5000); }
			 */
