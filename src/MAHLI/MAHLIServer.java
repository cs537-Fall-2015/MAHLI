...
public class MAHLIServer extends RoverServerRunnable {
	...
	public void run() {
		...
		ObjectOutputStream outputToAnotherServerObject;
		...
		try {
			while (true) {
				...
				//outputToAnotherServerObject = new ObjectOutputStream(get);

				// convert ObjectInputStream object to String
				String message = (String) inputFromAnotherObject.readObject();
				//String messageToServer = (String) inputFromAnotherServerObject.readObject();
				...
				case 16: // IMAGE_ANALYZE
					if (camOnStatus) {
						if (dataPath.listFiles().length > 0) {
							// File file = new
							// File(destination.toString()+"/"+destination.list()[0]);
							File file = new File(dataPath + "/data" + number + ".json");
							 ...
							 * System.out.println(readJSON.getJSONArray().size()
							 * + " - " + json.size()); JSONArray j =
							 * readJSON.getJSONArray();
							 * System.out.println(readJSON.getJSONArray().size()
							 * + " - " + j.size());
							 */
							...
							/*
							 * System.out.println(model.getDataName().size() +
							 * " - " + model.getData().size());
							 * System.out.println(model.getData());
							 * System.out.println(model.getDataName());
							 * System.out.println(readJSON.getDataName().size()
							 * + " - " + readJSON.getData().size()); BarChart
							 * bar = new BarChart();
							 * bar.setDataName(readJSON.getDataName());
							 * bar.setData(readJSON.getData()); PieChart pie =
							 * new PieChart();
							 ...
							 * ));
							 */
							displayCharts.displayApplet(); // FIX DEFAULT CLOSE
															// BUTTON
							cb.done();
						} else
							outputToAnotherObject.writeObject("Image needs to be Captured First");
